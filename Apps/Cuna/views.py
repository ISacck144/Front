...
class APIRootView(APIView):
    ...
    def get(self, request):
        return Response({
            ...
            'documentation': 'https://cuna-api-unsa-nine.vercel.app/api/',
            'timestamp': datetime.now().isoformat()
        })

class InitializeDataView(APIView):
    """Vista para inicializar datos en producción"""
    permission_classes = [AllowAny]
    ...
    def post(self, request):
        try:
            ...
            from rest_framework.authtoken.models import Token
            from Apps.Cuna.models.Student import Student
            from Apps.Cuna.models.Course import Course
            ...
            with connection.cursor() as cursor:
                ...
                results.append(f"✅ Conexión BD exitosa: {db_version}")
            
            # Crear superusuario si no existe
            if not User.objects.filter(username='admin').exists():
                admin_user = User.objects.create_superuser(
                    username='admin',
                    email='admin@cunaunsa.com',
                    password='AdminCuna2025'
                )
                results.append(f"✅ Admin creado: {admin_user.username}")
            else:
                results.append("ℹ️ Admin ya existe")
...
class HTTPMethodsTestView(APIView):
    """Vista para probar todos los métodos HTTP en Vercel"""
    permission_classes = [AllowAny]
    ...
    def put(self, request):
        return Response({
            'method': 'PUT',
            'success': True,
            'message': '✅ PUT funciona correctamente en Vercel',
            'data_received': request.data,
            'timestamp': datetime.now().isoformat()
        })
    ...
    def patch(self, request):
        return Response({
            'method': 'PATCH',
            'success': True,
            'message': '✅ PATCH funciona correctamente en Vercel',
            'data_received': request.data,
            'timestamp': datetime.now().isoformat()
        })
    ...
    def delete(self, request):
        return Response({
            'method': 'DELETE',
            'success': True,
            'message': '✅ DELETE funciona correctamente en Vercel',
            'timestamp': datetime.now().isoformat()
        })
...
class LogoutView(APIView):
    ...
    def post(self, request):
        #limpiamos la cookie de sesión (si la hubiera)
        logout(request)
